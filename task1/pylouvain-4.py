...
def in_order(nodes, edges):
        ...
        return (nodes_, edges_,node_list)
...
class PyLouvain:
    ...
    def from_file(cls, path):
        ...
        # rebuild graph with successive identifiers
        nodes_, edges_ ,node_list= in_order(nodes, edges)
        print("%d nodes, %d edges" % (len(nodes_), len(edges_)))
        #print(nodes)
        #print(nodes_)
        #print(edges)
        #print(edges_)
        return cls(nodes_, edges_),node_list
...
class PylouvainTest(unittest.TestCase):
    ...
    def test_relation(self):
        pyl ,nodes= PyLouvain.from_file("relationship.csv")
        partition, q = pyl.apply_method()
        file = open('community.dat', 'wb')
        print(len(partition))
        for community in partition:
            result=[]
            if len(community)>500:
                for paper in community:
                    result.append(nodes[paper])
                print(result)
